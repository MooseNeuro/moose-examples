...
def make_channel_CaHVA(parent='/library', name='CaHVA'):
    ...
    mgate.alpha = f'~(alpha:={alpha}, {Q10_MUL} * alpha)'
    beta = (
        '100 * ((v - 10e-3) - (-8.9e-3))/(-0.005) /'
        ' (1 - exp(200 * ((v - 10e-3) - (-8.9e-3))/(-0.005)))'
    )
    mgate.beta = f'~(beta:={beta}, {Q10_MUL} * beta)'
    alpha = '(v - 10e-3) < -0.060? 5.0: 5 * exp(-50 * ((v - 10e-3) - (-0.06)))'
    hgate.alpha = f'~(alpha:={alpha},{Q10_MUL} * alpha)'
    beta = (
        '(v - 10e-3) < -0.060? 0.0:'
        ' (5 - 5 * exp(-50 * ((v - 10e-3) - (-0.060))))'
    )
    hgate.beta = f'~(beta:={beta},{Q10_MUL} * beta)'
    return chan
...
def make_channel_KA(parent='/library', name='KA'):
    ...
    mgate.tau = f'~(tau:={tau}, {Q10_MUL} * tau)'
    ...
    hgate.tau = f'~(tau:={tau}, {Q10_MUL} * tau)'
...
def make_channel_KCa(parent='/library', name='KCa'):
    ...
    mgate.alpha = f'~(alpha:={alpha}, {Q10_MUL} * alpha)'  # Why 2500 instead of 1250 in OSB/NeuroML2
...
def make_channel_NaF(parent='/library', name='NaF'):
    ...
    alpha = f'{Q10_MUL} * 1500 * exp(81 *((v - 10e-3) - (-39e-3)))'
    beta = f'{Q10_MUL} * 1500 * exp(-66 * ((v - 10e-3) - (-39e-3)))'
    mgate.inf = f'~(alpha:={alpha}, beta:={beta}, alpha/(alpha+beta))'
    ...
    hgate.inf = f'~(alpha:={alpha}, beta:={beta}, alpha/(alpha+beta))'
