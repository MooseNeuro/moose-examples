...
def read_prototype(celltype, cdict):
    ...
    filename = os.path.join(config.modelSettings.protodir, f'{celltype}.p')
    logger.debug('Reading prototype file %s' % (filename))
    adjust_chanlib(cdict)
    cellpath = f'{config.modelSettings.libpath}/{celltype}'
    ...
    compartment_count = len(
        moose.wildcardFind(f'{proto.path}/#[TYPE=Compartment]')
    )
    config.logger.info(
        f'Read {celltype} with {compartment_count} compartments'
    )
    return proto
...
def assign_depths(cell, depthdict, leveldict):
    ...
    for level, depth in list(depthdict.items()):
        ...
        for comp_number in complist:
            assert cell.path
            comp = moose.element(f'{cell.path}/comp_{comp_number}')
            comp.z = z
    ...
    def __new__(cls, name, bases, cdict):
        if name != 'CellBase':
            ...
            if annotation is not None:
                ...
                info.notes = '\n'.join(
                    '"%s": "%s"' % kv for kv in list(annotation.items())
                )
            if 'soma_tauCa' in cdict:
                moose.element(proto.path + '/comp_1/CaPool').tau = cdict[
                    'soma_tauCa'
                ]
    ...
    def dump_cell(self, file_path):
        ...
        with open(file_path, 'w') as dump_file:
            ...
            writer = csv.DictWriter(
                dump_file, fieldnames=fieldnames, delimiter=','
            )
            writer.writeheader()
